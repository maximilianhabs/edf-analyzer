...
def compute_rr(path, channel):
    ...
    def local_pct_filter(rr_arr, half_win=6, max_dev=0.25):
        ...
        Sensitiver als Hampel für abrupte Einzelausreißer (Artefakt-R-Peaks)."""
        n = len(rr_arr)
        mask = np.ones(n, dtype=bool)
        for i in range(n):
            lo = max(0, i - half_win)
            hi = min(n, i + half_win + 1)
            # Nachbarn ohne den aktuellen Punkt für unverfälschten Median
    ...
    # Lokaler Prozent-Filter: entfernt abrupte Einzelausreißer (Artefakt-R-Peaks)
    # die Hampel und globale Grenzen noch durchlassen (z.B. 400ms bei Median 900ms)
...
def render():
    ...
    with col_tach:
        st.markdown("**Tachogramm**" + ("  — Rohdaten + Ausreißer markiert" if show_raw else " — bereinigt"))
        fig_rr = go.Figure()
        if show_raw:
    ...
    with col_poin:
        st.markdown("**Poincaré-Plot**" + ("  — Rohdaten + Ausreißer markiert" if show_raw else " — bereinigt"))
        fig_poin = go.Figure()
        if show_raw and len(rr_raw) > 1:
            ...
            fig_poin.add_trace(go.Scatter(
                ...
                name="betrifft Ausreißer", marker=dict(color="#c0392b", size=6, symbol="x", opacity=0.7),
    ...
    # ── Segmentierung für HV-Dateien ──────────────────────────────────────
    # Bei HV: rr_ms / r_times für Normwertanalyse auf Prä-HV-Daten begrenzen
    ...
    fd_burg = compute_frequency_domain(rr_ms_analysis, r_times_analysis, method="burg", burg_order=BURG_ORDER_DEFAULT)
    ...
    def render_psd_chart(fd_x, title, line_color):
        ...
        # Peak-Frequenz-Linien (NeuroFax-analog: LF/HF Hz-Gipfel)
        lf_pf = fd_x.get("lf_peak_freq")
        hf_pf = fd_x.get("hf_peak_freq")
        lf_pp = fd_x.get("lf_peak_psd")
        hf_pp = fd_x.get("hf_peak_psd")
    ...
    with col_psd_w:
        ...
        _lf_w = fd_welch.get("lf_peak_freq"); _hf_w = fd_welch.get("hf_peak_freq")
    ...
    with col_psd_b:
        ...
        _lf_b = fd_burg.get("lf_peak_freq"); _hf_b = fd_burg.get("hf_peak_freq")
    ...
    freq_method = st.radio(
        "Welche Methode für den HRV-Befund unten verwenden?",
        ["Welch (FFT)", "Burg (Maximum Entropy Method)"],
        horizontal=True,
        help="FFT/Welch: klassisch, robust, glättet über Fensterung — empfohlener Standard. "
             "Burg/MEM: schätzt ein autoregressives Modell direkt aus den Daten, ohne Fensterung — "
             "höhere Detailauflösung, aber empfindlicher gegenüber kurzen/unruhigen Segmenten.",
    )
    ...
    def _render_lab_panel(rr_seg: np.ndarray, r_times_seg: np.ndarray,
        ...
        """
        Rendert Balance-Gauge + alle 8 HRV-Parameterbalken für ein RR-Segment.
        sdnn_warning: SDNN-Wert anzeigen aber mit Hinweis (z.B. während HVT).
        freq_warning: Frequenzdomäne-Werte mit Hinweis (kompromittiert).
        Gibt (pdf_lab_rows, metrics_dict) zurück.
        """
        if len(rr_seg) < 5:
        ...
        # Frequenzdomäne für dieses Segment
        _fd = None
        ...
        if _fd and _fd["hf_power"] > 0 and not (float("nan") == _fd["lf_hf_ratio"]):
            try:
                ...
                _fig_bal = go.Figure()
                for x0b, x1b, cb in [(-100,-40,"#f1c40f"),(-40,-15,"#f1c40f"),(-15,15,"#27ae60"),(15,40,"#f1c40f"),(40,100,"#f1c40f")]:
                    _fig_bal.add_vrect(x0=x0b, x1=x1b, fillcolor=cb, opacity=0.18 if abs(x0b)==100 else 0.10 if abs(x0b)==40 else 0.18, line_width=0)
                ...
                _fig_bal.add_trace(go.Scatter(
                    ...
                    marker=dict(symbol="diamond", size=20, color="#2c3e50", line=dict(width=2, color="white")),
                ...
                _fig_bal.update_layout(
                    xaxis=dict(range=[-100,100], tickvals=[-100,-40,0,40,100],
                               ticktext=["stark vagal","leicht vagal","ausgeglichen","leicht symp.","stark symp."]),
        ...
        # Frequenzdomäne-Zusatzparameter (NeuroFax-analog)
        ...
        # HF-Band-Validitätswarnung: wenn Atemfrequenz > 0.40 Hz (= >24/min),
        # wandert der RSA-Gipfel aus dem standardisierten HF-Band heraus →
        # absolute HF-Power ist dann biologisch ungültig (nicht als Vaguskollaps werten).
        ...
        # Dreistufige Hierarchie: Validität → Zeitbereich → Frequenzbereich
        # Ebene 1 prüft ob die Basis für Ebene 3 überhaupt gegeben ist.
        lab_groups = [
        ...
        def _render_param_row(key, label, value, unit, _pid=panel_id):
            """Rendert eine einzelne Parameter-Zeile (Balken + Caption)."""
            is_nan = (value != value)
            ...
            with row_r:
                ...
                # Zonendefinition je Parameter-Typ
                if key == "lf_hf_ratio":
                    if is_pediatric:
                    ...
                    if pnn50_exp is not None and pnn50_exp >= 2.0:
                        ...
                        # Beide Werte sehr niedrig — alles als normal markieren
                    ...
                    if key == "heart_rate":
                        zones = [
                            (0,    40,   "pathologisch"),   # schwere Bradykardie
                            (40,   60,   "grenzwertig"),    # Bradykardie
                            (60,  100,   "normal"),         # Normbereich
                            (100, 140,   "grenzwertig"),    # Tachykardie
                            (140, cls["scale_max"], "pathologisch"),  # schwere Tachykardie
                        ]
                    else:
                        zones = [
                ...
                for x0, x1, zname in zones:
                    if x1 > x0:
                        fig_row.add_vrect(x0=x0, x1=x1, fillcolor=ZONE_COLOR[zname], opacity=0.22, line_width=0)

                if key == "lf_hf_ratio":
                    fig_row.add_annotation(x=0, y=1.35, xref="x", yref="paper", showarrow=False,
                                           text="◀ parasympathikoton", font=dict(size=10, color="#1a5276"), xanchor="left")
                    fig_row.add_annotation(x=cls["scale_max"], y=1.35, xref="x", yref="paper",
                                           showarrow=False, text="sympathikoton ▶", font=dict(size=10, color="#c0392b"), xanchor="right")
                if key in ("lf_norm", "hf_norm"):
                    lbl_lo = "vagal" if key == "lf_norm" else "symp."
                    lbl_hi = "symp." if key == "lf_norm" else "vagal"
                    fig_row.add_annotation(x=0, y=1.35, xref="x", yref="paper", showarrow=False,
                                           text=f"◀ {lbl_lo}", font=dict(size=10, color="#1a5276"), xanchor="left")
                    fig_row.add_annotation(x=cls["scale_max"], y=1.35, xref="x", yref="paper",
                                           showarrow=False, text=f"{lbl_hi} ▶", font=dict(size=10, color="#c0392b"), xanchor="right")
                ...
                # pNN50: zusätzlicher Konkordanz-Balkenvergleich (gemessen vs. erwartet)
                if key == "pnn50":
                    pnn50_exp = cls.get("pnn50_expected")
                    if pnn50_exp is not None and pnn50_exp > 0.5:
                        ...
                        fig_conc.update_layout(
                            xaxis=dict(range=[0, bar_scale], title="pNN50 (%)", tickfont=dict(size=10)),
                ...
                # Caption mit Wert + Zone + Normgrenze
                ref_src = "Gąsior 2018" if is_pediatric else "Hansen 2024"
                if cls["zone"] == "info":
                    ref_range = (f"Mayer-Wellen ~0.07–0.12 Hz"
    ...
    # ── Block 1: Prä-HV (oder Gesamtaufnahme bei Routine-EEG) ──────────────
    if is_pediatric:
        st.info(
    ...
    pdf_lab_rows, metrics_pre = _render_lab_panel(rr_ms_analysis, r_times_analysis, panel_id="pre")

    # fig_bal für PDF-Export (wurde in _render_lab_panel inline gezeichnet; hier rekonstruieren)
    balance = compute_autonomic_balance(rmssd, fd["hf_power"], fd["lf_hf_ratio"])

    fig_bal = go.Figure()
    fig_bal.add_vrect(x0=-100, x1=-40, fillcolor="#f1c40f", opacity=0.18, line_width=0)
    fig_bal.add_vrect(x0=-40, x1=-15, fillcolor="#f1c40f", opacity=0.10, line_width=0)
    fig_bal.add_vrect(x0=-15, x1=15, fillcolor="#27ae60", opacity=0.18, line_width=0)
    fig_bal.add_vrect(x0=15, x1=40, fillcolor="#f1c40f", opacity=0.10, line_width=0)
    fig_bal.add_vrect(x0=40, x1=100, fillcolor="#f1c40f", opacity=0.18, line_width=0)
    ...
    # fig_bal für PDF-Export rekonstruieren (wurde in _render_lab_panel inline gezeichnet)
    balance = compute_autonomic_balance(rmssd, fd["hf_power"], fd["lf_hf_ratio"])
    fig_bal = go.Figure()
    for x0b, x1b, cb, op in [(-100,-40,"#f1c40f",0.18),(-40,-15,"#f1c40f",0.10),
                               (-15,15,"#27ae60",0.18),(15,40,"#f1c40f",0.10),(40,100,"#f1c40f",0.18)]:
        fig_bal.add_vrect(x0=x0b, x1=x1b, fillcolor=cb, opacity=op, line_width=0)
    fig_bal.add_vline(x=0, line_color="#888", line_width=1, line_dash="dot")
    fig_bal.add_trace(go.Scatter(x=[balance["index"]], y=[0], mode="markers+text",
        marker=dict(symbol="diamond", size=20, color="#2c3e50", line=dict(width=2, color="white")),
        text=[f"{balance['index']:+.0f}"], textposition="top center", textfont=dict(size=13, color="#2c3e50")))
    fig_bal.update_layout(
        xaxis=dict(range=[-100,100], tickvals=[-100,-40,0,40,100],
                   ticktext=["stark vagal","leicht vagal","ausgeglichen","leicht symp.","stark symp."]),
        yaxis=dict(visible=False, range=[-1,1]),
        height=140, margin=dict(t=10,b=30,l=20,r=20), plot_bgcolor="white", showlegend=False)
...
---
#### Frequenzbereich — Normiert & Balance *(neu)*
...
---
#### Frequenzbereich — Gipfelfrequenzen *(neu, NeuroFax-analog)*
...
Physiologische Grundlage: Der HF-Peak der HRV entspricht der respiratorischen Sinusarrhythmie
(RSA) — das Herz beschleunigt bei Einatmung und verlangsamt bei Ausatmung; dieser
Rhythmus erscheint als Gipfel im HF-Spektrum.
Normbereich Ruhe: **12–20 /min** (0.20–0.33 Hz).
Klinische Bedeutung: Tachypnoe >20/min sichtbar als nach rechts verschobener HF-Peak;
während Hyperventilation verlässt der Peak das HF-Band nach oben (>0.4 Hz), wodurch
HF Power artifziell abfällt — nicht als autonome Suppression zu werten.
Quelle: **Yasuma F & Hayano J** (2004). *Respiratory sinus arrhythmia — why does the
...
Frequenz des dominanten Peaks im LF-Band (0.04–0.15 Hz). Entspricht den Mayer-Wellen —
Schwingungen im arteriellen Blutdruck, die durch den Baroreflex erzeugt werden.
Typisch: **0.07–0.12 Hz**. Keine etablierten klinischen Cutoffs.
Klinische Bedeutung: Verschobene Mayer-Wellen-Frequenz kann auf veränderte
Baroreflex-Sensitivität hindeuten; rein deskriptiv anzusehen.
Quelle: **Eckberg DL** (1997). *Sympathovagal balance — a critical appraisal.*
Circulation 96(9):3224–3232.
NeuroFax-Bezeichnung: **LF (Hz)**.

---
**Quellen:**
1. **Task Force ESC/NASPE** (1996). *Heart rate variability: standards of measurement,
   physiological interpretation, and clinical use.* Circulation 93(5):1043–1065.
2. **Hansen CS et al.** (2024). *Normative data on measures of cardiovascular autonomic
   neuropathy...* Clin Auton Res 35:101–113.
3. **Billman GE** (2013). *The LF/HF ratio does not accurately measure cardiac
   sympatho-vagal balance.* Front Physiol 4:26.
4. **Yasuma F & Hayano J** (2004). *Respiratory sinus arrhythmia.* Chest 125(2):683–690.
5. **Eckberg DL** (1997). *Sympathovagal balance — a critical appraisal.* Circulation 96(9):3224–3232.
6. **Gąsior JS et al.** (2018). *Standardization of the pediatric HRV.* Front Physiol 9:1495.
7. **Mietus JE et al.** (2002). *The pNNx files: re-examining a widely used HRV measure.* Heart 88(4):378–380. — Grundlage der RMSSD→pNN50-Konkordanzformel.

**Zonen-Logik (Erwachsene):**
- 🔴 Pathologisch: unter P5 alters-/HF-adjustiert (Hansen-Formel)
- 🟡 Grenzwertig: P5 bis 1.5×P5 — pragmatische Annäherung (echte P10-Formeln nicht verfügbar)
- 🟢 Normal: darüber
...
erfüllen diese Bedingungen nicht — alle Werte sind **Orientierung**, keine Diagnosekriterien.
    ...
    # Routine-EEG ohne HV: nur wenn User explizit aktiviert
    ...
    if has_hv and not _manual_active:
        hv_dur = ((phases["hvt_end"] or 0) - (phases["hvt_start"] or 0))
        # Warnung bei sehr kurzer HVT (< 60 s) — könnte Artefakt-Annotation sein
        if hv_dur < 60:
            st.warning(
        ...
        with col_info:
            st.info(
                ...
                + (f"Foto: {len(phases['photo_events'])} Schritte" if phases["has_photo"] else "")
            )
        with col_btn:
            st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
        ...
        # Manuelle Eingabe — HR-Plot vorab als Orientierung zeigen
        if not has_hv:
        ...
        # Schnell-HR-Plot zur visuellen Orientierung
        ...
        # Slider-Defaults: Auto-Werte wenn vorhanden, sonst Schätzung aus Aufnahmedauer
        ...
        with col_reset:
            if has_hv and st.button("↩ Auto", use_container_width=True, key="hvt_reset_btn"):
                st.session_state[_manual_key] = False
                st.rerun()

        # phases-Dict mit manuellen Werten überschreiben
        phases = dict(phases)
        phases["has_hv"]      = True
    ...
    # HR-Verlaufsplot mit Phasenbändern (bleibt sichtbar über alle Tabs)
    ...
    if seg_pre:
        fig_hr.add_hline(y=seg_pre["mean_hr"], line_dash="dot",
                         ...
                         annotation_font_size=10, annotation_font_color="#2980b9")
    ...
    # Frequenzdomäne für alle drei Phasen berechnen
    ...
    # Vergleichstabelle Prä/HVT/Post mit Zeitbereich + Frequenzdomäne
    def _seg_row(label, seg, fd, sdnn_warn=False):
        if seg is None:
            return {"Phase": label, "HR (bpm)": "—", "SDNN (ms)": "—",
                    "RMSSD (ms)": "—", "pNN50 (%)": "—",
                    "LF (ms²)": "—", "HF (ms²)": "—", "LF/HF": "—", "Schläge": "—"}
        ...
        return {
            ...
            "LF (ms²)": _fmt(fd["lf_power"] if fd else None, 0),
            "HF (ms²)": _fmt(fd["hf_power"] if fd else None, 0),
            "LF/HF": _fmt(fd["lf_hf_ratio"] if fd else None, 2),
            "Schläge": seg["n_beats"],
        }
    ...
    if seg_post and hvt_e and post_e:
        cmp_rows.append(_seg_row(f"Post-HV ({hvt_e:.0f}–{post_e:.0f}s)", seg_post, fd_post))
    ...
    if seg_post and seg_pre:
        ...
        st.markdown(
            f"<div style='padding:10px 14px;border-radius:8px;border:1px solid {rb['color']}55;"
            f"background:{rb['color']}11;margin:6px 0'>"
    ...
    with tabs[tab_idx]:
        ...
        if seg_hvt:
            _render_lab_panel(seg_hvt["rr_ms"], seg_hvt["r_times"],
                              sdnn_warning=True, freq_warning=True, panel_id="hvt")
        else:
            st.warning("Zu wenige Schläge im HVT-Segment.")

    # Tab: Post-HV Erholung
    if seg_post:
        with tabs[tab_idx]:
            tab_idx += 1
            st.markdown(
            ...
            _render_lab_panel(seg_post["rr_ms"], seg_post["r_times"], panel_id="post")
    ...
    if phases["has_photo"]:
        with tabs[tab_idx]:
            ...
            fig_ph.add_trace(go.Scatter(
                ...
                yaxis="y2", name="HR", line=dict(color="#2c3e50", width=1),
                hovertemplate="t=%{x:.1f}s  HR=%{y:.1f} bpm<extra></extra>",
            ))
            ...
            if seg_photo:
                ...
                st.info("Zu wenige Schläge im Fotostimulations-Segment für Parameterberechnung.")
