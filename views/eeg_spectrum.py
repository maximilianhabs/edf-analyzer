...
# Die spektralen Grundrechnungen liegen seit 2026-08-12 in `analysis/spectral.py` — sie
# gehören in die Analyseschicht, nicht in eine UI-Seite. `analysis/report_export.py` und
# `analysis/glory_report.py` importierten sie vorher von hier, die Analyse hing damit an der
# Oberfläche. Hier nur noch hereingeholt, damit der bestehende Seitencode unverändert bleibt.
from analysis.spectral import (  # noqa: F401  (Weiterverwendung im Seitencode)
    FREQ_MAX, _band_power, _compute_psd, _epoch_starts, _highpass, _peak_freq,
    _peak_freq_cog, _spectral_edge,
)
